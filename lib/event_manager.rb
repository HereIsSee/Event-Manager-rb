...
def clean_phone_number(phone_number)
  ...
  return 'The given number is invalid!' if phone_number.nil? || digit_count > 11 || digit_count < 10
  return 'The given number is invalid!' if digit_count == 11 && phone_number[0] != 1
  return remove_first_digit_in_string(phone_number) if digit_count == 11 && phone_number[0] == 1

  phone_number
end

def digit_count_in_string(string)
  string.chars.reduce(0) do |sum, value|
    sum += 1 if value.=~(/\d/)
    sum
  end
end
...
def count_registration_number_at_day(array_of_days, day)
  array_of_days[day] += 1
end

def peak_registration_hours(array_of_hours)
  array_of_hours.sort_by { |_key, value| value }.reverse.map { |key, _value| key }[0..2]
end
...
contents.each do |row|
  ...
  count_registration_number_at_day(day_registration_counter, registration_date.strftime('%A'))
